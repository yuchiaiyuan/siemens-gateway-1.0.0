...
class WebScreenshot:
    def __init__(self, config):
        ...
        self.url = config.get('url', 'https://www.baidu.com')
        self.save_dir = config.get('save_dir', './screenshots')
        self.cron_expression = config.get('cron_expression', "0 0/1 * * *",)  # 默认每小时执行一次
        self.width = config.get('width', 1920)
        self.height = config.get('height', 1080)
        self.wait_time = config.get('wait_time', 5)  # 等待网页加载时间
        self.chat_id = config.get('chat_id')
        self.title = config.get('title')
        self.executable_path = config.get('executable_path',r"C:\Program Files\Google\Chrome\Application\chrome.exe")
    ...
    async def take_screenshot(self):
        ...
        try:
            ...
            browser = await playwright.chromium.launch(
                headless=True,  # 无头模式
                executable_path="C:\Program Files\Google\Chrome\Application\chrome.exe",  # 替换为实际路径
                args=[
                    '--no-sandbox',
            ...
            feishu_send_img.main(self.chat_id,save_path,self.title,f'** 时间: **{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',"")
...
if __name__ == '__main__':
    ...
    # 创建截图实例并启动
    screenshot = WebScreenshot(config)
    screenshot.start()
